...
function selectOption(option) {
  ...
  if (option.nextOption === 0) {
    ...
    if (state.livesLeft <= 0 && state.deathCount >= 3) {
      return gameOver();
    }
  }
...
function updateDeathCountDisplay() {
  deathCountElement.innerText = `Deaths: ${state.deathCount}`;
}

function updateLivesLeftDisplay() {
  livesLeftElement.innerText = `Lives Left: ${state.livesLeft}`;
}
